...
Data:
Stack = an array
Register = current integer value (not part of the stack array)
Program commands will be made via a string passed in as an argument.

Algorithm:
Create a method minilang that takes a string as an argument. First, the method initializes the stack variable to an empty array and register variable alue to 0. Then, we can create a variable called instructions and assign that to the string split into an array of words. Iterate through the instructions array and convert each integer string into a number. Then, create a case statement for each instruction in the instructions list that calls seperate methods. Build a method for each instruction in the instructions list if needed to help complete the instruction.
...
def minilang(string)
  ...
  instructions.each do |instruction|
    register = instruction if (POSITIVE_NUMBERS.include?(instruction)) ||( NEGATIVE_NUMBERS.include?(instruction))
